...
final class WebP {
    ...
    private static String lib = "/tmp/libwebp.so";

    static synchronized void loadNativeLibrary () {
        if (!NATIVE_LIBRARY_LOADED) {
            File sofile = new File (WebP.class.getClassLoader ().getResource ("libwebp.so").getFile ());
            try {
                FileOutputStream outputStream = new FileOutputStream (lib);
                outputStream.write (Files.readAllBytes (sofile.toPath ()));
                System.load (lib);
